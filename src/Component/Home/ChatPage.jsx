...
const ChatPage = (props) => {
  ...
  return (
    <div className={"fkdjkfjkdlsasa"} style={{ display: "flex" }}>
      ...
      {
        isCall=== true && <VideoCallComponent channelName={callId} />
      }
      <MainChat setCallId={setCallId} isCall={isCall} setIsCall={setIsCall} setExtend={setExtend} setChange={setChange} conversation={conversation} />
      {
...
const StorageFile= ()=> {
  ...
  return (
    ...
    <div style={{width: "100%", display: "flex", alignItems: "center", flexWrap: "wrap"}}>
      {
        activePage=== false && data?.media?.filter(item=> item?.type_message=== "image")?.map((item, key)=> <div style={{width: "calc(100% / 3)", padding: 5}} key={key}>
          <img src={item.message} style={{width: "100%", aspectRatio: 1 / 1}} alt="" />
        </div>)
      }
      {
        activePage=== true && data?.media?.filter(item=> item?.type_message=== "file")?.map((item, key)=> <div key={key} style={{width: "calc(100% / 3)", padding: 5, height: 50, position: "relative"}}>
          <a target={"_blank"} style={{width: "100%", height: '100%'}} href={item.message} rel="noreferrer">
            <div style={{position: "absolute", top: 0, left: 0, width: '100%', height: '100%', border: '1px solid #e7e7e7'}} className={"c-flex-center"}>{item.name_file}</div>
          </a>
        </div>)
      }
    </div>
